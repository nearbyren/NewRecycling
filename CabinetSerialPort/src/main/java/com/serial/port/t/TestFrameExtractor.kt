 ...
 */
class TestFrameExtractor(private val onFrameFound: (ByteArray) -> Unit) {
    ...
    fun push(input: ByteArray) {
        ...
        try {
            ...
            if (currentTime - lastProcessTime > PROCESS_TIMEOUT && buffer.size() > 0) {
                println("push Parsing timeout, resetting buffer")
                 buffer.reset()
            }
            ...
            println("push Parsing exceptions: ${e.message}")
            buffer.reset()
        }
    }
    ...
    private fun processBuffer(currentTime: Long) {
        ...
        while (currentIndex < currentData.size) {
            println("push $currentIndex | ${currentData.size} | ${ByteUtils.toHexStringFastTo(currentData)}")
            // A. 查找帧头 0x9B
            val headerIndex = findFrameHeader(currentData, currentIndex)
            println("push headerIndex $headerIndex")

            if (headerIndex == -1) {
                // 全缓冲区没有帧头，全部标记为已处理
                lastValidEnd = currentData.size
                println("push There's a lot of noise behind it")
                break
            }
            ...
            if (headerIndex + POS_DATA_LEN >= currentData.size) {
                ...
                println("push Not enough data to read the length bit")
                // 如果这个半包在缓冲区停留超过了 HALF_FRAME_MAX_STAY，强制跳过
                if (currentTime - lastProcessTime > HALF_FRAME_MAX_STAY) {
                    println("push [ discard ] instruction bit Miss Packet has stayed too long")
            ...
            if (totalFrameLen > MAX_FRAME_SIZE || totalFrameLen < MIN_FRAME_SIZE) {
                println("push [ skip ] illegal packet length: $totalFrameLen")
                currentIndex = headerIndex + 1
                lastValidEnd = currentIndex
                continue
            }
            ...
            if (headerIndex + totalFrameLen > currentData.size) {
                ...
                if (currentTime - lastProcessTime > HALF_FRAME_MAX_STAY) {
                    println("push [Forced slingshot] Half packet delay is too high(${currentData.size}/$totalFrameLen)，Discard the old head")
                    currentIndex = headerIndex + 1
                    lastValidEnd = currentIndex
                    continue
                }
            ...
            if (currentData[frameEndIndex] != SendByteData.RE_FRAME_END) {
                println("push Frame end check failed, skip this frame header")
                currentIndex = headerIndex + 1
                lastValidEnd = currentIndex
                continue
            }
            ...
            if (validateCheckSum(packet)) {
                // --- 校验通过，执行回调 ---
                println("push headerIndex 校验通过，执行回调")
                onFrameFound(packet)
                // 推进指针到包末尾
                currentIndex = headerIndex + totalFrameLen
                lastValidEnd = currentIndex
            } else {
                println("push [Checksum] failure")
                currentIndex = headerIndex + 1
                lastValidEnd = currentIndex
            }
        }
        ...
        // 4. 清理已处理的数据，保留残留数据
        buffer.reset()
        println("push 保留残留前 $lastValidEnd | ${currentData.size} ")
        if (lastValidEnd < currentData.size) {
            val remaining = currentData.copyOfRange(lastValidEnd, currentData.size)
            println("push 保留残留 $lastValidEnd | ${currentData.size} | ${ByteUtils.toHexStringFastTo(remaining)}")
