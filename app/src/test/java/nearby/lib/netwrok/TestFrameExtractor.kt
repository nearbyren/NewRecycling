package nearby.lib.netwrok
 ...
 import com.serial.port.t.TestFrameExtractor
...
//        val extractor = FrameExtractor { results.add(it) }
        val extractor = TestFrameExtractor { results.add(it) }
...
//        )
        val stickyData = byteArrayOf(
            0x9B.toByte(), 0x00.toByte(), 0x01.toByte(), 0x02.toByte(), 0x01.toByte(), 0x01.toByte(), 0xA0.toByte(), 0x9A.toByte(),
            0x9B.toByte(), 0x00.toByte(), 0x05.toByte(), 0x1A.toByte(),
            0x01.toByte(), 0x00.toByte(), 0x00.toByte(), 0x28.toByte(), 0x3C.toByte(), 0x00.toByte(), 0x01.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte(), 0x01.toByte(), 0x00.toByte(),
            0x02.toByte(), 0xFF.toByte(), 0xFF.toByte(), 0xFA.toByte(), 0xEC.toByte(), 0x00.toByte(), 0x01.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte(), 0x01.toByte(), 0x01.toByte(), 0x00.toByte(),
    ...
    private val extractor = TestFrameExtractor { frames.add(it) }
    ...
    fun testID5PacketLossRecovery() {
        ...
        for(i  in  1.. 100000){
        val partialID5 = ByteUtils.hexToBytes("9B00051A010000283C000100000000010002FFFFFAEC00010000000101000A9A")
        extractor.push(partialID5)
        println("send 包1 ${frames.size}")
        Thread.sleep(500)


        val partialID6 = ByteUtils.hexToBytes("9B00051A010000283C000100000000010002FFFFFAEC00010000000101000A9A")
        extractor.push(partialID6)
        println("send 包2 ${frames.size}")
        Thread.sleep(1000)


        val partialID7 = ByteUtils.hexToBytes("9B00051A010000283C000100000000010002FFFFFAEC00010000000101000A9A")
        extractor.push(partialID7)
        println("send 包3 ${frames.size}")

        val partialID8 = ByteUtils.hexToBytes("9A")
        extractor.push(partialID8)
        println("send 包4 ${frames.size}")
        Thread.sleep(1200)
