...
object CalculationUtil {
    ...
    fun subtractFloats(after: String, before: String): String {
        ...
        return bd1.subtract(bd2).setScale(2, RoundingMode.HALF_DOWN).toString()
    }
    /***
    ...
    fun lessEqual(after: String): Boolean {
        val bd1 = BigDecimal(after).setScale(2, RoundingMode.HALF_DOWN).toDouble()
        val bd2 = BigDecimal("0.10").setScale(2, RoundingMode.HALF_DOWN).toDouble()
        return bd1 <= bd2
    }
    ...
    fun addFloats(num1: String, num2: String): String {
        val bd1 = BigDecimal(num1)
        val bd2 = BigDecimal(num2)
        return bd1.add(bd2).setScale(2, RoundingMode.HALF_DOWN).toString()
    }
    /***
     * 相乘
     * @param after
    ...
    fun multiplyFloats(after: String, before: String): String {
        val bd1 = BigDecimal(after)
        val bd2 = BigDecimal(before)
        return bd1.multiply(bd2).setScale(2, RoundingMode.HALF_DOWN).toString()
    }
    /***
     * 相除
     * @param after
    ...
    fun divideFloats(after: String, before: String): String {
        val bd1 = BigDecimal(after)
        val bd2 = BigDecimal(before)
        return bd1.divide(bd2, 2, RoundingMode.HALF_DOWN).toString()
    }
